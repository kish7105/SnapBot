...
async def exception_manager(
    interaction: Interaction, error: app.AppCommandError
) -> None:
    ...
    if isinstance(error, app.CommandOnCooldown):
        await interaction.response.send_message(
            format_as_error_msg(
                f"Woah.. calm down buddy! Retry after ``{error.retry_after: .2f} seconds``"
            ),
            ephemeral=True,
        )
    ...
    # If the bot lacks permissions to execute a command
        ...
        await interaction.response.send_message(
            format_as_error_msg(
                "I don't have the necessary permissions to use this command!"
            ),
            ephemeral=True,
        )
    ...
    # If the command invoker invokes a command in DMs which is restricted to guild only
        ...
        await interaction.response.send_message(
            format_as_error_msg(
                "This command is not available for use in DMs! Use it in a server channel instead."
            ),
            ephemeral=True,
        )
    ...
    # If the url provided by the command invoker is not Valid/Discord CDN URL
        ...
        await interaction.response.send_message(
            format_as_error_msg(
                "The URL is not valid! Make sure it starts with ``https://cdn.discordapp.com/``"
            ),
            ephemeral=True,
        )
    ...
    # If the command invoker invokes an owner-only command
        ...
        await interaction.response.send_message(
            format_as_error_msg("This command can only be invoked by the owner!"),
            ephemeral=True,
        )
    ...
    # For any unknown/misc exceptions...
        ...
        await interaction.response.send_message(
            format_as_error_msg(
                "An unknown error occurred during the command execution. Please try again later!"
            ),
            ephemeral=True,
        )
