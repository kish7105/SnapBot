...
class AuthorTextModal(Modal, title="About Embed Editor"):
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        if await coll.find_one({"user_id": interaction.user.id}) is None:
            await coll.insert_one({"user_id": interaction.user.id, "author_text": self.author_text.value})

        else:
            await coll.update_one(
                {"user_id": interaction.user.id}, {"$set": {"author_text": self.author_text.value}}
            )
