...
class DescriptionModal(Modal, title="About Embed Editor"):
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        if await coll.find_one({"user_id": interaction.user.id}) is None:
            await coll.insert_one({"user_id": interaction.user.id, "description": self.description.value})

        else:
            await coll.update_one(
                {"user_id": interaction.user.id}, {"$set": {"description": self.description.value}}
            )
